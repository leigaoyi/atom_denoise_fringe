...
parser.add_argument('--image', type=str, default='./tmp/atom_10.tif')
parser.add_argument('--refImage', type=str, default='./tmp/ref_10.tif')
parser.add_argument('--mask', type=str, default='')
parser.add_argument('--output', type=str, default='./tmp/pred_atom_10.tif')
...
try:  # for unexpected error logging
    with torch.no_grad():   # enter no grad context
        if is_image_file(args.image):
            ...
            refImage = io.imread(args.refImage)
            maxRef = refImage.max()
            minRef = refImage.min()
            ## change to tif output
            #vutils.save_image(inpainted_result, args.output, padding=0, normalize=True)
            inpaintedArray = inpainted_result.cpu().detach().numpy()
            print(inpaintedArray.shape)
            inpaintedResult = np.asarray(inpaintedArray[:,0,:,:], dtype=np.float32)
            inpaintedResult = (inpaintedResult - inpaintedResult.min())/(inpaintedResult.max()-inpaintedResult.min())
            inpaintedResult = inpaintedResult * (maxRef-minRef)+minRef
            inpaintedResult = np.asarray(inpaintedResult, dtype=np.uint32)
            io.imsave(args.output, inpaintedResult)
